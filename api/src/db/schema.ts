...
export const openrouterRequests = sqliteTable('openrouter_requests', {
  ...
  statusCode: integer('status_code'), durationMs: integer('duration_ms').notNull(), errorMessage: text('error_message'), sourceUrl: text('source_url'), isFavorite: integer('is_favorite', { mode: 'boolean' }).notNull().default(false), clientIp: text('client_ip'), clientColo: text('client_colo'),
...
export const walletSignupGrants = sqliteTable('wallet_signup_grants', { userId: integer('user_id').primaryKey().references(() => users.id, { onDelete: 'cascade' }), walletLedgerEntryId: integer('wallet_ledger_entry_id').notNull().unique().references(() => walletLedgerEntries.id, { onDelete: 'restrict' }), createdAt: timestamp() })
