...
- **Vertical Japanese text**: `jpn_vert` is bundled and the worker loads `jpn+jpn_vert`, because Tesseract's `jpn` config declares `tessedit_load_sublangs jpn_vert` but does not pull it in on its own. The first pass always uses `PSM_SINGLE_BLOCK` and the popup shows that horizontal read right away next to a `Ngang` / `Dọc` switch. A selection taller than it is wide is pre-read in the background with `PSM_SINGLE_BLOCK_VERT_TEXT` (`readVertical()`), while a wider one skips the pre-read and the switch triggers the read on click. Every read goes through `queueOcr()` — one shared worker means an unsynchronised PSM change would leak into the next read.
