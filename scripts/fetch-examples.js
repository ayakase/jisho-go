...
const SOURCE_JSON = path.join(ROOT, "src/assets/lib/dict-ver-2.json");
const OUTPUT_JSON = path.join(ROOT, "src/assets/lib/dict-ver-3.json");
...
const BASE_HEADERS = {
  ...
  referer: "https://mazii.net/vi-VN/search/example/javi/",
  "sec-ch-ua":
...
async function fetchExampleData(kanji) {
  const response = await fetch("https://mazii.net/api/search/example", {
    method: "POST",
    headers: {
      ...BASE_HEADERS,
      cookie: COOKIES.join("; "),
  ...
  if (!response.ok) {
    throw new Error(
      `Failed to fetch examples for "${kanji}": ${response.status} ${response.statusText}`
    );
  }
...
async function main() {
  ...
  for (const [index, entry] of entries.entries()) {
    ...
    if (!kanji) {
      ...
      enriched.push({ ...entry, examples: [] });
      continue;
    }

    try {
      console.log(
        `[${index + 1}/${entries.length}] Fetching examples for "${kanji}"...`
      );
      const exampleResults = await fetchExampleData(kanji);
      enriched.push({ ...entry, examples: exampleResults });
      console.log(`  Found ${exampleResults.length} example(s) for "${kanji}"`);
    } catch (error) {
      console.error(`Error fetching examples for "${kanji}":`, error);
      enriched.push({ ...entry, examples: [], error: String(error) });
    }
  }
