...
const SOURCE_JSON = path.join(ROOT, "src/assets/lib/dict-ver-1.json");
const OUTPUT_JSON = path.join(ROOT, "src/assets/lib/dict-ver-2.json");
...
const BASE_HEADERS = {
  ...
  referer: "https://mazii.net/vi-VN/search/grammar/javi/",
  "sec-ch-ua":
...
async function fetchGrammarData(kanji) {
  const response = await fetch("https://mazii.net/api/search/grammar", {
    method: "POST",
    headers: {
      ...BASE_HEADERS,
      cookie: COOKIES.join("; "),
  ...
  if (!response.ok) {
    throw new Error(
      `Failed to fetch grammar for "${kanji}": ${response.status} ${response.statusText}`
    );
  }
...
async function main() {
  ...
  for (const [index, entry] of entries.entries()) {
    ...
    if (!kanji) {
      ...
      enriched.push({ ...entry, grammar: [] });
      continue;
    }

    try {
      console.log(
        `[${index + 1}/${entries.length}] Fetching grammar for "${kanji}"...`
      );
      const grammarResults = await fetchGrammarData(kanji);
      enriched.push({ ...entry, grammar: grammarResults });
      console.log(
        `  Found ${grammarResults.length} grammar result(s) for "${kanji}"`
      );
    } catch (error) {
      console.error(`Error fetching grammar for "${kanji}":`, error);
      enriched.push({ ...entry, grammar: [], error: String(error) });
    }
